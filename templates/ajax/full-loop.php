...
if ( $posts_array ) :
	...
	foreach ( $posts_array as $post1 ) : ?>
		...
		<div class="single-note">
			...
			<?php
				$was_notified = get_post_meta( $post1->ID, 'llms_notify_admin', true );
			?>

			<?php
				$related_post_id = get_post_meta( $post1->ID, 'related_post_id', true );
			?>
			...
			<?php if ( $admin_response ) : ?>
				<div class="admin_response"><?php echo wp_kses_post( wpautop( $admin_response ) ); ?></div>
			<?php endif; ?>
			...
			if ( (int) $user_id === (int) $post1->post_author ) :
				...
				<div class="del">
					...
					<a class="note-lesson-link" href="<?php echo esc_attr( get_permalink( $related_post_id ) ); ?> " title="<?php echo esc_attr( get_the_title( $related_post_id ) ); ?>">
						<?php echo esc_html( get_option( 'opentext' ) ); ?>
